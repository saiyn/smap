 ...
 */
#define SAIYN_TELOPT_SGA	3
#define SAIYN_TELOPT_STATUS	5


#define SAIYN_GA 	249
#define SAIYN_WILL	251
#define SAIYN_DO 	253
#define SAIYN_IAC 	255
...
void telnet_event_handler(nsock_pool nsp, nsock_event nse, void *mydata) {
  ...
  char cmd[9] = {SAIYN_IAC, SAIYN_DO, SAIYN_TELOPT_SGA, SAIYN_IAC,SAIYN_WILL,24, SAIYN_IAC,SAIYN_DO,1};
  ...
  if (status == NSE_STATUS_SUCCESS) {
    ...
    case NSE_TYPE_CONNECT_SSL:
      ...
      ts->latest_readtcpev = nsock_read(nsp, ts->tcp_nsi, telnet_event_handler, read_timeout, ts);
      ts->latest_readstdinev = nsock_readlines(nsp, ts->stdin_nsi, telnet_event_handler, read_timeout, ts, 1);
      ts->latest_writetcpev = nsock_write(nsp, ts->tcp_nsi, telnet_event_handler, write_timeout, ts, &cmd[0],9);
      break;
    case NSE_TYPE_READ:
      str = nse_readbuf(nse, &nbytes);
      if (nsi == ts->tcp_nsi) {
        //printf("%s", str);
        printf("Read from tcp socket (%d bytes):%s\n", nbytes, str);
